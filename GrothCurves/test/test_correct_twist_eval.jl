...
# Correct twist isomorphism evaluation for M-twist
# x_(E') = x_(E) * v
...
function eval_line_via_twist_isomorphism(coeffs::GrothCurves.LineCoeffs, P::G1Point)
    ...
    # ψ(P):   X = xP * v     (in the c0 limb)
    #         Y = yP         (but multiplied by w when placed into Fp12)
    axv = a6 * Fp6Element(xP, zero(Fp2Element), zero(Fp2Element)) * v_fp6
    c0 = axv + c6

    # b*yP goes to the w-limb directly (NO extra v!)
    c1 = b6 * Fp6Element(yP, zero(Fp2Element), zero(Fp2Element))

    return Fp12Element(c0, c1)
end

# Miller loop using corrected twist evaluation
function miller_loop_correct_twist(P::G1Point, Q::G2Point)
    ...
    for i in (length(loop_bits)-1):-1:1
        ...
        T = T_new
        f = f^2 * eval_line_via_twist_isomorphism(line_coeffs, P)
        
        # Addition step if bit is 1
        if loop_bits[i]
            T_new, line_coeffs = GrothCurves.addition_step(T, Q)
            T = T_new
            f = f * eval_line_via_twist_isomorphism(line_coeffs, P)
        end
    end
    ...
    f = f * eval_line_via_twist_isomorphism(line1, P)
    
    Q_pi2 = GrothCurves.frobenius_g2(Q, 2)
    neg_Q_pi2 = -Q_pi2
    T2, line2 = GrothCurves.addition_step(T1, neg_Q_pi2)
    f = f * eval_line_via_twist_isomorphism(line2, P)
    
    return f
end

function pairing_correct_twist(P::G1Point, Q::G2Point)
    f = miller_loop_correct_twist(P, Q)
    return GrothCurves.final_exponentiation(f)
end

println("=== Testing with CORRECTED twist isomorphism (no extra v on y) ===\n")

P = g1_generator()
Q = g2_generator()

e = pairing_correct_twist(P, Q)

# Test negation first
println("Negation tests:")
e_negP = pairing_correct_twist(-P, Q)
e_negQ = pairing_correct_twist(P, -Q)
println("  e(-P, Q) == inv(e(P, Q)): ", e_negP == inv(e))
println("  e(P, -Q) == inv(e(P, Q)): ", e_negQ == inv(e))

# Test bilinearity
println("\nBilinearity tests:")
e_2P = pairing_correct_twist(2*P, Q)
e_P2Q = pairing_correct_twist(P, 2*Q)
e_squared = e^2
...
O1 = zero(G1Point)
O2 = zero(G2Point)
println("  e(O, Q) == 1: ", isone(pairing_correct_twist(O1, Q)))
println("  e(P, O) == 1: ", isone(pairing_correct_twist(P, O2)))

# General bilinearity
println("\nGeneral bilinearity:")
e_3P_5Q = pairing_correct_twist(3*P, 5*Q)
e_15 = e^15
