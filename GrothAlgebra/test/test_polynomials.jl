...
@testset "Polynomial Tests" begin
    # Use BN254Fq for testing
    F = BN254Fq
    
    @testset "Polynomial Construction" begin
        # Test basic construction
        coeffs = [bn254_fq(1), bn254_fq(2), bn254_fq(3)]
        p = Polynomial(coeffs)
        ...
        coeffs_with_zeros = [bn254_fq(1), bn254_fq(2), bn254_fq(0)]
        r = Polynomial(coeffs_with_zeros)
        @test length(r.coeffs) == 2
        
        # Test zero polynomial
        zero_poly = Polynomial([bn254_fq(0)])
        @test is_zero(zero_poly)
    end
    
    @testset "Polynomial Properties" begin
        # Test degree
        p = Polynomial([bn254_fq(1), bn254_fq(2), bn254_fq(3)])
        @test degree(p) == 2
        ...
        @test leading_coefficient(p) == bn254_fq(3)
        
        # Test predicates
        @test is_zero(zero_poly)
        @test !is_zero(p)
        
        constant_poly = Polynomial([bn254_fq(5)])
        @test is_constant(constant_poly)
        @test !is_constant(p)
        
        monic_poly = Polynomial([bn254_fq(2), bn254_fq(1)])
    ...
    @testset "Polynomial Equality" begin
        p1 = Polynomial([bn254_fq(1), bn254_fq(2)])
        p2 = Polynomial([bn254_fq(1), bn254_fq(2)])
        p3 = Polynomial([bn254_fq(1), bn254_fq(3)])
    ...
    @testset "Polynomial Arithmetic" begin
        p1 = Polynomial([bn254_fq(1), bn254_fq(2)])      # 1 + 2x
        p2 = Polynomial([bn254_fq(3), bn254_fq(4)])      # 3 + 4x
        
        # Test addition
        sum_poly = p1 + p2
        expected_sum = Polynomial([bn254_fq(4), bn254_fq(6)])  # 4 + 6x
        @test sum_poly == expected_sum
        
        # Test subtraction
        diff_poly = p1 - p2
        expected_diff = Polynomial([bn254_fq(-2), bn254_fq(-2)])  # -2 - 2x
        @test diff_poly == expected_diff
        
        # Test negation
        neg_poly = -p1
        expected_neg = Polynomial([bn254_fq(-1), bn254_fq(-2)])
        @test neg_poly == expected_neg
        
        # Test multiplication
        prod_poly = p1 * p2
        # (1 + 2x)(3 + 4x) = 3 + 4x + 6x + 8x^2 = 3 + 10x + 8x^2
        expected_prod = Polynomial([bn254_fq(3), bn254_fq(10), bn254_fq(8)])
        @test prod_poly == expected_prod
        
        # Test scalar multiplication
        scalar = bn254_fq(3)
        scalar_prod = p1 * scalar
        expected_scalar = Polynomial([bn254_fq(3), bn254_fq(6)])
        @test scalar_prod == expected_scalar
        @test scalar * p1 == scalar_prod
    end
    
    @testset "Polynomial Exponentiation" begin
        p = Polynomial([bn254_fq(1), bn254_fq(1)])  # 1 + x
        ...
        p_squared = p^2
        # (1 + x)^2 = 1 + 2x + x^2
        expected = Polynomial([bn254_fq(1), bn254_fq(2), bn254_fq(1)])
    ...
    @testset "Polynomial Evaluation" begin
        p = Polynomial([bn254_fq(1), bn254_fq(2), bn254_fq(3)])  # 1 + 2x + 3x^2
        
        # Test evaluation at specific points
        @test evaluate(p, bn254_fq(0)) == bn254_fq(1)
        @test evaluate(p, bn254_fq(1)) == bn254_fq(6)  # 1 + 2 + 3
        @test evaluate(p, bn254_fq(2)) == bn254_fq(17)  # 1 + 4 + 12
        
        # Test callable syntax
        @test p(bn254_fq(1)) == bn254_fq(6)
        
        # Test evaluation at multiple points
        points = [bn254_fq(0), bn254_fq(1), bn254_fq(2)]
        values = evaluate(p, points)
        expected_values = [bn254_fq(1), bn254_fq(6), bn254_fq(17)]
        @test values == expected_values
    end
    
    @testset "Polynomial Interpolation" begin
        # Test simple linear interpolation
        points = [bn254_fq(0), bn254_fq(1)]
        values = [bn254_fq(1), bn254_fq(3)]
        ...
        single_p = interpolate([bn254_fq(1)], [bn254_fq(5)])
        @test is_constant(single_p)
        @test evaluate(single_p, bn254_fq(0)) == bn254_fq(5)
        
        # Test error cases
        @test_throws ArgumentError interpolate(F[], F[])
        @test_throws ArgumentError interpolate([bn254_fq(1)], [bn254_fq(1), bn254_fq(2)])
    end
    
    @testset "Polynomial Derivative" begin
        # Test derivative of 3 + 2x + x^2 should be 2 + 2x
        p = Polynomial([bn254_fq(3), bn254_fq(2), bn254_fq(1)])
        dp = derivative(p)
        expected = Polynomial([bn254_fq(2), bn254_fq(2)])
        @test dp == expected
        
        # Test derivative of constant
        const_p = Polynomial([bn254_fq(5)])
        @test is_zero(derivative(const_p))
        
        # Test derivative of linear
        linear_p = Polynomial([bn254_fq(1), bn254_fq(2)])
        linear_dp = derivative(linear_p)
        @test linear_dp == Polynomial([bn254_fq(2)])
    end

    @testset "Roots of unity" begin
        roots = roots_of_unity(8, BN254Fr)
        @test length(roots) == 8
        ω = roots[2]
        oneF = one(BN254Fr)
    ...
    @testset "FFT interpolation" begin
        ...
        values = [bn254_fr(i) for i in 1:4]
    ...
    @testset "Coset FFT" begin
        offset = bn254_fr(5)
        domain = EvaluationDomain(BN254Fr, 8; offset=offset)
        coeffs = [bn254_fr(i) for i in 0:7]
        evals = fft(coeffs, domain)
        points = Vector{BN254Fr}(undef, domain.size)
